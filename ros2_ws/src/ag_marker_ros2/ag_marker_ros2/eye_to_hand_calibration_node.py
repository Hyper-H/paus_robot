...
# 导入 json，用于发布结构化状态。
import json
# 导入 dataclass，便于保存单次样本。
from dataclasses import dataclass
# 导入 Path，便于处理输出路径。
from pathlib import Path

# 导入 OpenCV 和 NumPy，用于棋盘格检测与矩阵计算。
import cv2
import numpy as np
# 导入 ament 索引，用于定位默认配置目录。
from ament_index_python.packages import get_package_share_directory
# 导入 cv_bridge，用于 ROS 图像到 OpenCV 图像转换。
...
class CalibrationSample:
    # 当前样本计算得到的 base->camera 变换矩阵。
    base_to_camera_matrix: np.ndarray


# ROS2 节点：采集 eye-to-hand 标定样本并求解外参。
class EyeToHandCalibrationNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter("output_path", "/root/projects/ag-repro/configs/extrinsics.yaml")
