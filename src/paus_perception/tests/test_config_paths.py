...
from paus_perception.config import resolve_config_artifact_path, resolve_config_path, resolve_runtime_data_path
...
def test_ros_share_prefix_paths_resolve_under_runtime_dir(monkeypatch, tmp_path: Path) -> None:
    runtime_dir = tmp_path / "runtime"
...
def test_ros_share_prefix_single_component_config_paths_resolve_under_runtime_dir(monkeypatch, tmp_path: Path) -> None:
    runtime_dir = tmp_path / "runtime"
    config_path = Path("/opt/ros/humble/share/paus_bringup/configs/default.yaml")
    monkeypatch.setenv("PAUS_ROBOT_RUNTIME_DIR", str(runtime_dir))

    assert Path(resolve_config_path("calibration_sessions", config_path)) == runtime_dir / "calibration_sessions"
