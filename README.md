...
?????ROS2??????FAIRINO SDK ??? UI ??????? Linux ????????? WSL ????????????

```bash
ssh chen_lab@192.168.58.183
cd ~/worktrees/paus_robot_handeye
source /opt/ros/humble/setup.bash
source ~/miniconda3/etc/profile.d/conda.sh
conda activate paus_robot
...
UI server ???? `0.0.0.0:8080`?Windows ?????????????
...
1. ???? dry-run ?? UI????????overlay?waypoint ??????????????????
2. ?? `/api/status`?`/api/handeye/quality`?`/api/sessions` ?????WebSocket ???????????
3. ????????? `execute_motion: true` ???????????????????
4. ?????? 3 ? 5 ? waypoint??????????
