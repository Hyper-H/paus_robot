...
def _launch_setup(context, *args, **kwargs):
    ...
    save_sample_images = _as_bool(LaunchConfiguration("save_sample_images").perform(context))
    ...
    execute_motion = _as_bool(LaunchConfiguration("execute_motion").perform(context))
...
def generate_launch_description() -> LaunchDescription:
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument("board_cols", default_value=str(calibration_cfg.get("board_cols", 9)), description="Chessboard inner-corner cols."),
            DeclareLaunchArgument("square_size_m", default_value=str(calibration_cfg.get("square_size_m", 0.01)), description="Chessboard square size in meters."),
            DeclareLaunchArgument("solver_method", default_value=str(calibration_cfg.get("solver_method", "joint_absolute")), description="Hand-eye calibration solver method."),
            DeclareLaunchArgument("min_sample_count", default_value=str(calibration_cfg.get("min_sample_count", 10)), description="Minimum number of valid samples before solve."),
            DeclareLaunchArgument("output_path", default_value=str(calibration_cfg.get("output_path", bringup_share / "configs" / "extrinsics.yaml")), description="Output extrinsics.yaml path."),
            DeclareLaunchArgument("trajectory_path", default_value=str(calibration_cfg.get("trajectory_path", default_config_file.parent / "eye_to_hand_trajectory.yaml")), description="Semi-auto trajectory YAML path."),
            DeclareLaunchArgument("session_root_path", default_value=str(calibration_cfg.get("session_root_path", "calibration_sessions")), description="Calibration session archive root."),
            DeclareLaunchArgument("save_sample_images", default_value=str(calibration_cfg.get("save_sample_images", True)).lower(), description="Whether to save accepted sample images."),
            DeclareLaunchArgument("max_reprojection_error_px", default_value=str(calibration_cfg.get("max_reprojection_error_px", 0.0)), description="Max single-image reprojection error. 0 disables this filter."),
            DeclareLaunchArgument("min_board_margin_px", default_value=str(calibration_cfg.get("min_board_margin_px", 10.0)), description="Minimum board margin in pixels."),
            DeclareLaunchArgument("stable_position_tolerance_mm", default_value=str(calibration_cfg.get("stable_position_tolerance_mm", 0.2)), description="TCP stability position tolerance."),
            DeclareLaunchArgument("stable_rotation_tolerance_deg", default_value=str(calibration_cfg.get("stable_rotation_tolerance_deg", 0.1)), description="TCP stability rotation tolerance."),
            DeclareLaunchArgument("stable_window_s", default_value=str(calibration_cfg.get("stable_window_s", 0.5)), description="Required stable window in seconds."),
            DeclareLaunchArgument("stable_timeout_s", default_value=str(calibration_cfg.get("stable_timeout_s", 10.0)), description="Timeout for waiting stable TCP."),
            DeclareLaunchArgument("dwell_s", default_value=str(calibration_cfg.get("dwell_s", 0.5)), description="Extra dwell after reaching each waypoint."),
            DeclareLaunchArgument("execute_motion", default_value="false", description="Whether to allow real robot motion. Defaults to dry-run."),
            DeclareLaunchArgument("tool_to_board_tx", default_value=str(default_tool_to_board_translation[0]), description="Tool-to-board x in meters."),
            DeclareLaunchArgument("tool_to_board_ty", default_value=str(default_tool_to_board_translation[1]), description="Tool-to-board y in meters."),
            DeclareLaunchArgument("tool_to_board_tz", default_value=str(default_tool_to_board_translation[2]), description="Tool-to-board z in meters."),
